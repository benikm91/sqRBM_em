...
class QBMBase(ABC):
    ...
    def __init__(self, n_visible, n_hidden, seed, W_init, b_init, Gamma_init):
        """
        ...
        self.seed = seed
        self.W_init = W_init.copy()
        self.b_init = b_init.copy()
        self.Gamma_init = Gamma_init.copy()
        self.rng = get_rng(self.seed)
        self.grads = {}

        self._initialize_weights_and_biases(self.W_init, self.b_init, self.Gamma_init)

    def _apply_grads_quantum(self, learning_rate):
        """
        ...
        self.Gamma[self.n_visible :] += learning_rate * (self.grads["Gamma_pos"] - self.grads["Gamma_neg"])

    def _apply_grads_classical(self, learning_rate):
        """
        Applies the gradients from the positive and negative phases using the provided
        learning rate.
